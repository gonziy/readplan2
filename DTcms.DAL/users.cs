...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        public bool ExistsWxOpenID(string openid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from " + databaseprefix + "users");
            strSql.Append(" where wx_open_id=@wx_open_id ");
            SqlParameter[] parameters = {
                    new SqlParameter("@wx_open_id", SqlDbType.VarChar,100)};
            parameters[0].Value = openid;
            int count = TypeConverter.ObjectToInt(DbHelperSQL.GetSingle(strSql.ToString(), parameters));
            return count > 0 ? true : false;
        }

        public List<Model.users> GetModelByWx(string openid)
        {
            List<Model.users> list = DbHelperSQL.ListEntity<Model.users>("select top 3 * from [" + databaseprefix + "users] where wx_open_id='" + openid + "' and status<3");
            return list;
        }
        /// <summary>
        /// 检查用户名是否存在
        ...
        public int Add(Model.users model)
        {
            ...
            strSql.Append("group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,wx_open_id,is_show)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@wx_open_id", SqlDbType.VarChar,50),
            ...
            parameters[21].Value = model.wx_open_id;
            parameters[22].Value = model.is_show;
        ...
        public bool Update(Model.users model)
        {
            ...
            strSql.Append("wx_open_id=@wx_open_id");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@group_id", SqlDbType.Int,4),
					new SqlParameter("@user_name", SqlDbType.NVarChar,100),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,wx_open_id,is_pay");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,wx_open_id,is_pay,is_show");
            strSql.Append(" FROM " + databaseprefix + "users ");
            if (strWhere.Trim() != "")
            {
        ...
        public List<Model.users> List(int Top, string strWhere, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (Top > 0)
            ...
            strSql.Append(" order by " + filedOrder);
            return DbHelperSQL.ListEntity<Model.users>(strSql.ToString());
        }
        ...
        public Model.users DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["wx_open_id"] != null)
                {
                    model.wx_open_id = row["wx_open_id"].ToString();
                }

                if (row["is_pay"] != null && row["is_pay"].ToString() != "")
                {
                    model.is_pay = int.Parse(row["is_pay"].ToString());
                }
            }
            return model;
        }
