...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "compose":  //合并
                    compose(context);
                    break;
                case "show_next_account": //添加帐户
                    show_next_account(context);
                    break;

                case "send_remind": //未读提醒
                    send_remind(context);
                    break;
            }
        }
        private void send_remind(HttpContext context)
        {
            BLL.users usersBLL = new BLL.users();

            BLL.UserReadLog logBLL = new BLL.UserReadLog();
            BLL.weixin_template_msg msgBLL = new BLL.weixin_template_msg();
            int pageIndex = 1;
            int recordCount = 0;
            int pages = 0;
            DataSet ds = usersBLL.GetList(1, pageIndex, "status='0'", "id asc", out recordCount);
            pages = recordCount % 100 == 0 ? recordCount / 100 : recordCount / 100 + 1;
            for (int i = 1; i <= pages; i++)
            {
                ds.Clear();
                ds = usersBLL.GetList(100, i, "status='0'", "id asc", out recordCount);
                if (ds != null)
                {
                    if (ds.Tables.Count > 0)
                    {
                        if (ds.Tables[0].Rows != null)
                        {
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                {
                                    Model.users userModel = usersBLL.GetModel(TypeConverter.ObjectToInt(dr["id"]));
                                    string yesterday = DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd");
                                    int count = logBLL.Count("user_id='" + TypeConverter.ObjectToInt(dr["id"]) + "' and read_date='" + yesterday.Replace("-", "") + "' and plan_read_count>read_count");
                                    if (count > 0)
                                    {
                                        msgBLL.SendRemind(userModel.wx_open_id, "系统检测到您昨日读经计划未完成", "您昨日有未读完的经书，请您及时完成计划", yesterday, "【点击这里】打开读经系统，进行补读", "http://www.yuedujing.com/wx_login.aspx");
                                    }
                                }
                            }
                        }
            ...
            context.Response.Write("{\"status\":0, \"msg\":\"任务已执行\"}");
            return;
        }

        private void show_next_account(HttpContext context)
        {
            ...
            BLL.users usersBLL = new BLL.users();
            userModel = usersBLL.GetModel(userModel.id);
            if (userModel == null)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，用户不存在！\"}");
                return;
            }
            List<Model.users> users = usersBLL.List(100, "wx_open_id='" + userModel.wx_open_id + "'", "id asc");
            int showAccountCount = 0;
            int open = 0;
            foreach (Model.users user in users)
            {
                if (open == 0)
                {
                    if (user.is_show == 0)
                    {
                        int r = usersBLL.UpdateField(user.id, "is_show='1'");
                        open = 1;
                        showAccountCount++;
                    }
                }
            ...
            if (showAccountCount==users.Count)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"帐户都已开启！\"}");
                return;
            }
            else
            {
                context.Response.Write("{\"status\": 1, \"msg\": \"帐户添加成功！\"}");
                return;
            }
        ...
        private void compose(HttpContext context)
        {
            string user_id = DTRequest.GetFormString("user_id");
            string category_id = DTRequest.GetFormString("category_id");
            ...
            userModel = new BLL.users().GetModel(userModel.id);
            if (userModel == null)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，用户不存在！\"}");
                return;
            ...
            if (!Utils.isNumber(category_id))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，参数传输有误！\"}");
                return;
            }
            Model.article_category categoryModel = new BLL.article_category().GetModel(int.Parse(category_id));
            if (categoryModel == null)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，该书不存在！\"}");
                return;
            }
            int count = 0;
            List<Model.article> articles = new BLL.article().List(6, categoryModel.id, 10000, 1, "id>0", "id desc", out count);
            int composeArticleCount = 0;
            if (articles != null && articles.Count > 0)
            {
                BLL.UserReadLog userReadLogBLL = new BLL.UserReadLog();
                foreach (Model.article article in articles)
                {
                    int r = userReadLogBLL.ArticleCompose(userModel.id, article.id);
                    if (r > 0)
                        composeArticleCount++;
                }
            }
            if (composeArticleCount == count)
            {

                bool is_upgrade = false;

                //如果是整本加入整本表
                BLL.UserReadBook userReadBookBLL = new BLL.UserReadBook();
                Model.UserReadBook userReadBook = userReadBookBLL.Get("user_id='" + userModel.id + "' and category_id='" + categoryModel.id + "'");
                if (userReadBook != null && userReadBook.id != null && userReadBook.id > 0)
                {
                    userReadBookBLL.UpdateField(TypeConverter.ObjectToInt(userReadBook.id), "[count]=[count]+1");
                }
                else
                {
                    userReadBook = new Model.UserReadBook();
                    userReadBook.category_id = categoryModel.id;
                    userReadBook.count = 1;
                    userReadBook.user_id = userModel.id;
                    userReadBookBLL.Add(userReadBook);
                }

                //赠送积分
                int point = categoryModel.point;
                if (point > 0)
                {
                    new BLL.user_point_log().Add(userModel.id, userModel.user_name, point, "合成《" + categoryModel.title + "》送积分" + point, true, out is_upgrade);
                }

                if (is_upgrade)
                {
                    int group_id = new BLL.users().GetModel(userModel.id).group_id;
                    context.Response.Write("{\"status\": 99, \"msg\": \"合成成功！\",\"group_id\":"+ group_id + "}");
                    return;
                }
                else
                {
                    context.Response.Write("{\"status\": 1, \"msg\": \"合成成功！\"}");
                    return;
                }
            }
            else
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"无法合成,请检查是否已经合成成功！\"}");
                return;
            }
        }

        private void plan_read(HttpContext context)
        {
            string user_id = DTRequest.GetFormString("user_id");
            string category_id = DTRequest.GetFormString("category_id");
            string article_id = DTRequest.GetFormString("article_id");
            string read_count = DTRequest.GetFormString("read_count");
            string date = DTRequest.GetFormString("date_str");
            ...
            if (int.Parse(DateTime.Now.ToString("yyyyMMdd")) < int.Parse(date))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，您选择的日期超过当前日期！\"}");
                return;
            }
            BLL.UserReadLog userReadLog = new BLL.UserReadLog();
            List<Model.UserReadLog> list = new List<Model.UserReadLog>();
            string read_type = string.IsNullOrEmpty(article_id) ? "0" : "1";
            string article_sql = read_type == "1" ? " and [article_id]='" + article_id + "' " : "";
            list.AddRange(userReadLog.List("[read_type]='" + read_type + "' and [category_id]='" + category_id + "' " + article_sql + " and [user_id]='" + user_id + "' and [read_date]='" + date + "' and [read_count] < [plan_read_count]"));
            if (list != null)
            {
                if (list.Count > 0)
                {
                    Model.UserReadLog model = list[0];
                    model.read_count = TypeConverter.ObjectToInt(model.read_count, 0) + 1;
                    int r = userReadLog.Update(model, "[read_type]='" + read_type + "' and [category_id]='" + category_id + "' " + article_sql + " and [user_id]='" + user_id + "' and [read_date]='" + date + "' and [read_count] < [plan_read_count]");
                    if (r > 0)
                    {
                        bool is_upgrade = false;
                        //整本赠送积分
                        ...
                        {
                            int group_id = new BLL.users().GetModel(userModel.id).group_id;
                            context.Response.Write("{\"status\": 99, \"msg\": \"祝贺读完一遍！\",\"group_id\":" + group_id + "}");
                            return;
                        }
                        else
                        {
                            context.Response.Write("{\"status\": 1, \"msg\": \"祝贺读完一遍！\"}");
                            return;
                        }
                    }
                }
            ...
            context.Response.Write("{\"status\": 0, \"msg\": \"无法记录,也许您已经超出计划次数！\"}");
            return;
        }

        private void create_plan(HttpContext context)
        {
            string count = DTRequest.GetFormString("count");
            ...
            //判断用户的计划是否达到上限
            BLL.UserReadLog userReadLog = new BLL.UserReadLog();
            int month = int.Parse(DateTime.Now.ToString("yyyyMMdd"));
            int monthB = int.Parse(DateTime.Now.ToString("yyyyMM")+"01");
            int monthE = int.Parse(DateTime.Now.ToString("yyyyMM") + "31");
            List<Model.UserReadLog> isPlan = userReadLog.List("category_id='" + category_id + "' and user_id ='"+ userModel.id + "' and read_date>='"+monthB+ "' and read_date<='" + monthE + "'");
            if (isPlan.Count > 0)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，该书已经制定计划！\"}");
                return;
            }

            isPlan = userReadLog.List(" user_id ='" + userModel.id + "' and read_date>='" + monthB + "' and read_date<='" + monthE + "'");
            int booksCount = 0;
            int now_book_category_id = 0;
            foreach (Model.UserReadLog log in isPlan)
            {
                if (now_book_category_id != log.category_id)
                {
                    now_book_category_id = TypeConverter.ObjectToInt(log.category_id);
                    booksCount++;
                }
            }
            if (booksCount >= 3)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"对不起，每月最多做三本书的计划，不要贪心啊！\"}");
                return;
            }

            BLL.article article = new BLL.article();
            //判定计划时间（上月29-31 本月1-28 都可以制定本月计划）
            ...
            if (type == "book")
            {
                for (int i = 1; i <= 28; i++)
                {
                    Model.UserReadLog model = new Model.UserReadLog();
                    model.read_type = 0;
                    ...
                    {
                        //下月
                        yearAndMonth = DateTime.Now.AddMonths(1).ToString("yyyyMM");
                    }
                    else
                    {
                        //本月
                        yearAndMonth = DateTime.Now.ToString("yyyyMM");
                    }
                    ...
                    model.read_date = int.Parse(yearAndMonth + strDay);
                    model.read_count = 0;
                    model.plan_read_count = int.Parse(count) / 28;
                    model.add_time = DateTime.Now;
                    r += userReadLog.Add(model);
                }
            ...
            {
                int articleCount = 0;
                List<Model.article> articles = article.List(6, categoryModel.id, 10000, 1, "", "sort_id asc", out articleCount);
                int BookReadTimes = 28 / articles.Count;
                for (int i = 1; i <= 28; i++)
                {
                    string yearAndMonth = string.Empty;
                    if (day > 28)
                    ...
                    if (articleCount > 0 && articleCount <= 14)
                    {
                        int t = 0;
                        t = i% articles.Count == 0 ? i / articles.Count : i / articles.Count + 1;
                        int article_i = i - 1 - (t - 1) * articles.Count;
                        model.read_type = 1;
                        model.category_id = categoryModel.id;
                        model.article_id = articles[article_i].id;
                        model.user_id = userModel.id;
                        model.read_date = int.Parse(yearAndMonth + strDay);
                        model.read_count = 0;
                        model.plan_read_count = int.Parse(count);
                        model.add_time = DateTime.Now;
                        model.compose_count = 0;
                        r += userReadLog.Add(model);

                    }
                    else if (articleCount > 14 && articleCount <= 28)
                    {
                        if (i - 1 < articleCount)
                        {
                            model.read_type = 1;
                            model.category_id = categoryModel.id;
                            model.article_id = articles[i - 1].id;
                            model.user_id = userModel.id;
                            model.read_date = int.Parse(yearAndMonth + strDay);
                            model.read_count = 0;
                            model.plan_read_count = int.Parse(count);
                    ...
                    {
                        int everydayCount = (articleCount - (articleCount % 28)) / 28 + 1;
                        for (int b = 0; b < everydayCount; b++)
                        {
                            if ((everydayCount * i - everydayCount + b) < articleCount)
                            {
                                model.read_type = 1;
                                model.category_id = categoryModel.id;
                                model.article_id = articles[everydayCount * i - everydayCount + b].id;
                                model.user_id = userModel.id;
                                model.read_date = int.Parse(yearAndMonth + strDay);
                                model.read_count = 0;
                                model.plan_read_count = int.Parse(count);
            ...
            if (r > 0)
            {
                context.Response.Write("{\"status\": 1, \"msg\": \"计划已经生成！\"}");
                return;
            }
            else
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"计划生成失败！\"}");
                return;
            }
